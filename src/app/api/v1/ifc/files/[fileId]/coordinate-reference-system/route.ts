/**
 * GET /api/v1/ifc/files/[fileId]/coordinate-reference-system — get CRS
 */

import { NextRequest } from "next/server";
import { verifyTokenAndScope } from "@/modules/auth/authService";
import { getProject } from "@/modules/ifc/projectService";
import { getCoordinateReferenceSystem } from "@/modules/ifc/ifcModelService";
...
export async function GET(
  ...
  try {
    ...
    const crs = await getCoordinateReferenceSystem(fileId);
    return ok(crs);
  } catch (err) {
    if (err instanceof NotFoundError) return notFound(err.message);
    console.error("[GET CRS]", err);
