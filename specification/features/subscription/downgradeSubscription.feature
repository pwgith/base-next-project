...
Feature: Downgrade subscription
  ...
  Scenario: Paid plan user selecting downgrade is redirected to the Stripe Customer Portal
    Given the user is subscribed to the "Investor" plan with a billing period ending on "2026-04-06"
  ...
  Scenario: System records a scheduled downgrade after receiving a Stripe webhook
    Given the user is subscribed to the "Investor" plan with a billing period ending on "2026-04-06"
    When Stripe sends a subscription.updated webhook indicating a downgrade to "Hobby" at period end
    Then a downgrade to "Hobby" is recorded as scheduled for "2026-04-06"
    And the user's plan remains "Investor" until that date
  ...
  Scenario: System records a downgrade to Free after a Stripe cancellation webhook
    ...
    And the user's plan remains "Hobby" until that date
  ...
  Scenario: User returns from Stripe Customer Portal without making a change — plan unchanged
    ...
    And the user's plan remains "Investor"
