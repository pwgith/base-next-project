...
Feature: View subscription plans
  ...
  Scenario: Free plan user sees upgrade options but no downgrade or cancel option
    ...
    When the user views the subscription plans page
    Then upgrade options are available for the "Hobby" and "Investor" plans
    And no downgrade or cancel option is shown

  @S-070 @UC-USR-011
  Scenario: Hobby plan user sees upgrade to Investor and options to downgrade or cancel
    Given the user is subscribed to the "Hobby" plan
    When the user views the subscription plans page
    Then an upgrade option is available for the "Investor" plan
    And downgrade and cancel options are available

  @S-071 @UC-USR-011
  Scenario: Investor plan user sees downgrade and cancel options but no upgrade option
    Given the user is subscribed to the "Investor" plan
    When the user views the subscription plans page
  ...
  Scenario: Paid plan user sees current billing details sourced from Stripe
    Given the user is subscribed to the "Hobby" plan with a billing period ending on "2026-04-06"
    When the user views the subscription plans page
    Then the current plan is shown as "Hobby"
  ...
  Scenario: Paid plan user with a pending downgrade sees the scheduled change
    Given the user is subscribed to the "Investor" plan
    And Stripe reports a scheduled downgrade to "Hobby" effective "2026-04-06"
    When the user views the subscription plans page
    Then the pending downgrade to "Hobby" effective "2026-04-06" is displayed
  ...
  Scenario: Plan prices are displayed in Australian Dollars
    ...
    Then the plan prices are displayed in Australian Dollars:
      ...
      | Free     | A$0 / month      |
      | Hobby    | A$3.00 / month   |
      | Investor | A$15.00 / month  |

  @S-099 @UC-ADM-001
  Scenario: Plan prices are sourced from Stripe and reflect admin price changes
    Given the Stripe "Hobby" plan price is "A$3.00 / month"
    And the Stripe "Investor" plan price is "A$15.00 / month"
    When the user views the subscription plans page
    Then the "Hobby" plan price is displayed as "A$3.00 / month"
    And the "Investor" plan price is displayed as "A$15.00 / month"
